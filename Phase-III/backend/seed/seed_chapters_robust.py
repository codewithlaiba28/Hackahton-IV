...
async def seed_chapters():
    ...
    async with async_session() as session:
        # Phase 1: Create chapters without FKs
        for chapter_data in CHAPTERS:
            ...
            db_data = {k: v for k, v in chapter_data.items() if k not in ['content', 'prev_chapter_id', 'next_chapter_id']}
            chapter = Chapter(**db_data, r2_content_key=f"chapters/{chapter_data['id']}/content.md")
            session.add(chapter)
        
        await session.commit()
        print("Phase 1: Basic chapters created.")

        # Phase 2: Update FKs
        for chapter_data in CHAPTERS:
            await session.execute(
                update(Chapter)
                .where(Chapter.id == chapter_data["id"])
                .values(
        ...
        await session.commit()
        print("Phase 2: Chapter relationships updated.")
